...
def change_status_command(command: commands.ChangeStatusCommand, uow: unit_of_work.UnitOfWork):
    print('handle change status')

def handle_create_product(command: commands.CreateProductCommand, uow: unit_of_work.UnitOfWork):
...
def log_product_created(event: events.ProductCreated, uow: unit_of_work.UnitOfWork):
...
def log_product_created2(event: events.ProductCreated, uow: unit_of_work.UnitOfWork):
    # handle the event by logging the product created
